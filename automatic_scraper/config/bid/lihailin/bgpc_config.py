...
detail_pattern = {
    ...
    "sc": {'pattern': "//div[@class='content-right-details-content']", 'type': 'xpath', 'target': 'clean_html', 'custom_func_name': ''},
...
def process_list_item(list_element, item):
    ...
    """
    # 停止翻页
    if 'tid/2' in item['_current_start_url']:
        item['bid_type'] = 1
    elif 'tid/5' in item['_current_start_url']:
        item['bid_type'] = 0
    elif 'tid/4' in item['_current_start_url']:
        item['bid_type'] = 2
    if 'tid/6' in item['_current_start_url']:
        item['is_liubiao'] = 1
    else:
        item['is_liubiao'] =0
