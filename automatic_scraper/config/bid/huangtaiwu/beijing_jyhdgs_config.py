...
author = "huangtaiwu"
web_title = u"北京金亿宏达装饰工程有限公司"
data_source = 'http://www.jyhdgs.com'

start_urls = [
    "http://www.jyhdgs.com/html/news/",  # 1
]
...
# 列表页模板
index_pattern = {
    "_list": {'pattern': "//div[@class='bt']", 'type': 'xpath', 'target': 'html',
              'custom_func_name': ''},
    "_next_page": {'pattern': u"//a[@href and text()='下一页']", 'type': 'xpath', 'target': 'html',
                   'custom_func_name': ''},
    "title": {'pattern': "//a/h4", 'type': 'xpath', 'target': 'text', 'custom_func_name': ''},
...
# 详情页模板
detail_pattern = {
    "sc": {'pattern': "//div[@class='xqnr']", 'type': 'xpath', 'target': 'clean_html',
           'custom_func_name': ''},
...
def process_list_item(list_element, item):
    ...
    # 停止翻页
    item['bid_type'] = 1
    if re.search(u'中标公告', item['title']):
        item['bid_type'] = 0
...
def process_detail_item(item):
    ...
    """
    item['issue_time'] = re.search(u'【(.*?)】', item['issue_time']).group(1)
