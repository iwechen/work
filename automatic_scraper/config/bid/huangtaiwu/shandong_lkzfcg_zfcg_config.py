...
start_urls = [
    "http://www.lkzfcg.gov.cn/index.php?m=content&c=index&a=lists&catid=38",  # 1
    "http://www.lkzfcg.gov.cn/index.php?m=content&c=index&a=lists&catid=13",  # 0
]
...
# 列表页模板
index_pattern = {
    "_list": {'pattern': "//div[@class='content-box']//ul//li", 'type': 'xpath', 'target': 'html',
              'custom_func_name': ''},
...
def process_list_item(list_element, item):
    ...
    """
    # 停止翻页
    if re.search('38', item['_current_start_url']):
        item['bid_type'] = 1
    elif re.search('13', item['_current_start_url']):
        item['bid_type'] = 0
    else:
        item['bid_type'] = -1
