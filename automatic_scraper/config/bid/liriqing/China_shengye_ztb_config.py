...
author = "liriqing"
web_title = u"中国商业贸易网"
data_source = 'http://www.200806.com'

start_urls = [

     #政府采购
    "http://www.200806.com/news/dianzishuma-c73104-1.html",
...
# 列表页模板
index_pattern = {
    "_list": {'pattern': "//li[@class='f_gray']",'type': 'xpath', 'target': 'html', 'custom_func_name': ''},
...
detail_pattern = {
    ...
    "title": {'pattern': "/html//h1[@id='title']", 'type': 'xpath', 'target': 'text', 'custom_func_name': ''},
...
def process_list_item(list_element, item):
    ...
    """
    time_data=re.search(r'</span>(.*?)</li>',list_element,re.S).group(1)
...
def process_detail_item(item):
    ...
    """
    if u'招标|工程' not in item['title']:
        return
    item['bid_type'] = 1
    if re.search(ur'中标公示|结果|成交', item['title']):
        item['bid_type'] = 0
