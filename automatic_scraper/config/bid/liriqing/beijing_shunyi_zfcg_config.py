...
author = "liriqing"
web_title = u"北京市顺义区政府采购网"
data_source = 'http://www.caigou.bjshy.gov.cn'

start_urls = [
    #招标公告
    "http://www.caigou.bjshy.gov.cn/level2.jsp?caid=011",
...
# 列表页模板
index_pattern = {
    "_list": {'pattern': "//tr[@style='font-size:14px;']", 'type': 'xpath', 'target': 'html', 'custom_func_name': ''},
...
# 详情页模板
detail_pattern = {
    "sc": {'pattern': "//tr[@valign='top']", 'type': 'xpath', 'target': 'html', 'custom_func_name': ''},
...
def process_list_item(list_element, item):
    ...
    if re.search(ur'变更',item['title']):
        item['bid_type'] = 2
    if re.search(ur'中标', item['title']):
            item['bid_type'] = 0
    if re.search(ur'废标', item['title']):
            item['is_get'] = 1
    # 停止翻页
