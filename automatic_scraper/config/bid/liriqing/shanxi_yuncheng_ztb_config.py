...
author = "liriqing"
web_title = u"运城市水务局网"
data_source = 'http://www.sxycwater.gov.cn'

start_urls = [
    # 招标公告

     #政府采购
    "http://www.sxycwater.gov.cn/listall.asp?Classid=9",
...
# 列表页模板
index_pattern = {
    "_list": {'pattern': "//div[@class='listright']//li",'type': 'xpath', 'target': 'html', 'custom_func_name': ''},
...
# 详情页模板
detail_pattern = {
    "sc": {'pattern': "//div[@class='viewright']", 'type': 'xpath', 'target': 'html', 'custom_func_name': ''},
...
def process_detail_item(item):
    ...
    """
    if u'公告|公示|工程' not in item['title']:
        return
    if len(item['sc']) > 0:
        item['is_get'] = 1
    else:
